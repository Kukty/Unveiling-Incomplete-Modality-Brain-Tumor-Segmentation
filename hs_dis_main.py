...
from hs_dis_trainer import run_training
...
from models.nets import SwinUNETR
...
parser.add_argument("--in_modalities",nargs="+", type=str, default=["Flair", "T1", "T1c" ,"T2"], help="used modality of input")
...
parser.add_argument("--pretrain_teacher",default=None,type=str,help= "pretrain swinVit")
parser.add_argument(
    "--pretrained_dir",
...
parser.add_argument('--use_kl',default=True,help='Use kl loss')
parser.add_argument('--use_mse',default=True,help='Use mse loss')
parser.add_argument('--T',type=float,default=10,help='kd loss weight')
parser.add_argument("--pretrain_vit",default=None,type=str,help= "pretrain swinVit")

def main():
    args = parser.parse_args()
    args.in_channels = len(args.in_modalities)
...
def main_worker(gpu, args):
    ...
    if args.pretrain_vit is not None:
        model_dict = torch.load(args.pretrain_vit)["state_dict"]
        model.swinViT.load_state_dict(model_dict)
        print("Using pretrained Encoder")
    
    if args.pretrain_teacher is not None:
        model_dict = torch.load(args.pretrain_teacher)["state_dict"]
