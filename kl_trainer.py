...
from utils.loss import softmax_kl_loss,softmax_holder_loss
    ...
    for idx, batch_data in enumerate(loader):
        ...
        with autocast(enabled=args.amp):
            ...
            if args.use_kl:
                ...
                loss_kd = args.kd_weight * softmax_kl_loss(logits/args.T,logits_t/args.T).mean()
                loss = loss_dice+loss_kd
            elif args.use_holder:
                with torch.no_grad():
                    logits_t = teacher(data)
                loss_kd = args.kd_weight * softmax_holder_loss(logits/args.T,logits_t/args.T).mean()
                loss = loss_dice+loss_kd
            else :
                loss = loss_dice
...
def run_training(
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if args.rank == 0 and writer is not None:
            ...
            writer.add_scalar("kd_loss",kd_loss,epoch)
        b_new_best = False
